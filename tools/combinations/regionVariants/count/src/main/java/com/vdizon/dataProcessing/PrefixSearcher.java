...
public class PrefixSearcher {
    ...
    private void addContainsType(String name, String region) {
        ...
        for (int i = 0; i < mGroupPrefixes.length; i++) {
            ...
            addGenomeRegionIfMatches(parsed_string, name, genomeRegion, i);
        }
    }

    private void addGenomeRegionIfMatches(String[] prefixes, String genomeName, GenomeRegion region, int index) {
        for (String s : prefixes) {
            if (genomeName.startsWith(s)) {
                region.addTypeCount(index);
                return;
            }
        }

        region.addTypeCount(mGroupPrefixes.length);
